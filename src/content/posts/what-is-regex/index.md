...
| --- | --- | --- | --- |
| . | ドット | 任意の1文字 | [詳細](#2-2-1) |
| \[...\] | 文字クラス | リストの中の任意の1文字 | [詳細](#2-2-2) |
| \[^...\] | 否定文字クラス | リストに含まれていない任意の1文字 | [詳細](#2-2-3) |
| \\char | エスケープ文字 | charがメタ文字の場合はただの文字列として扱う | [詳細](#2-2-4) |
| ? | 疑問符 | 直前の文字が0個または1つ | [詳細](#2-3-1) |
| \* | スター | 直前の文字が0個以上 | [詳細](#2-3-2) |
| + | プラス | 直前の文字が1個以上 | [詳細](#2-3-3) |
| {min, max} | 範囲指定 | 直前の文字が min 個以上 max 個以下 | [詳細](#2-3-4) |
| ^ | キャレット | 行の先頭の位置 | [詳細](#2-4-1) |
| $ | ドル | 行の末尾の位置 | [詳細](#2-4-2) |
| (?=...) | 先読み | 右側に指定の文字列がある位置 | [詳細](#2-4-3) |
| (?<=...) | 後読み | 左側に指定の文字列がある位置 | [詳細](#2-4-3) |
| (?!...) | 否定先読み | 右側に指定の文字列がない位置 | [詳細](#2-4-3) |
| (?<!...) | 否定後読み | 左側に指定の文字列がない位置 | [詳細](#2-4-3) |
| \| | 選択 | 区切っている正規表現のどれか | [詳細](#2-5-1) |
| (...) | 括弧 | 1\. 選択の範囲を限定する   2\. 量指定子の対象範囲を指定する   3\. 後方参照のためにキャプチャする | [詳細](#2-5-2) |
