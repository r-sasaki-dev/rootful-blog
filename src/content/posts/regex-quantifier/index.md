...
- [正規表現で繰り返し回数を指定するにはどうしたらいい？](#1)

- [それぞれの量指定子の意味と使い方を知りたい](#2)
...
| --- | --- | --- | --- | --- |
| ? | ?? | 疑問符 | 直前の文字が0個または1つ | [\>>](#3-1) |
| \* | \*? | スター   （アスタリスク） | 直前の文字が0個以上 | [\>>](#3-2) |
| + | +? | プラス | 直前の文字が1個以上 | [\>>](#3-3) |
| {N} | \- | 範囲指定 | 直前の文字がN個 | [\>>](#3-4-1) |
| {min,} | {min,}? | 範囲指定 | 直前の文字が min 個以上 | [\>>](#3-4-2) |
| {,max} | {,max}? | 範囲指定 | 直前の文字が max 個以下 | [\>>](#3-4-3) |
| {min,max} | {min,max}? | 範囲指定 | 直前の文字が min 個以上 max 個以下 | [\>>](#3-4-4) |
